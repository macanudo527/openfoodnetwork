...
supplier_id: cakes_supplier.id) }
 ...
 create(:product, name: "cherries", primary_taxon_id: fruits.id, supplier_id: fruits_supplier.id) }
    let!(:product_doughnuts) {
 create(:product, name: "doughnuts", primary_taxon_id: cakes.id, supplier_id: cakes_supplier.id) }
 ...
 "#{cakes.id},#{fruits.id}" }
        ...
        expect(products).to eq([product_banana_bread, product_doughnuts, product_apples, 
product_cherries])
      end
...
{ q: { name_or_meta_keywords_or_variants_display_as_or_variants_display_name_or_supplier_name_cont: "apples" } })
        products = products_renderer.send(:products)
 ...
 Spree::Property.create! name: 'Conventional', presentation: 'Conventional' }
        ...
        it "filters products with a product property" do
          product_apples.product_properties.create!({ property_id: property_organic.id, value: '1', 
position: 1 })
          products_renderer = ProductsRenderer.new(distributor, order_cycle, customer, 
